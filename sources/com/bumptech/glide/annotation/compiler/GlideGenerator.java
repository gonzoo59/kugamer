...
public final class GlideGenerator {
    ...
    public MethodSpec overrideGlideStaticMethod(ExecutableElement executableElement) {
        ...
        MethodSpec.Builder addParameters = MethodSpec.methodBuilder(executableElement.getSimpleName().toString()).addModifiers(Modifier.PUBLIC, Modifier.STATIC).addJavadoc(this.processorUtil.generateSeeMethodJavadoc(executableElement)).addParameters(parameters);
        addReturnAnnotations(addParameters, executableElement);
        boolean z = asElement != null;
        if (z) {
            addParameters.returns(ClassName.get(asElement));
        }
    ...
    private MethodSpec.Builder addReturnAnnotations(MethodSpec.Builder builder, ExecutableElement executableElement) {
        String obj = this.processingEnv.getElementUtils().getTypeElement(this.processorUtil.visibleForTesting().reflectionName()).toString();
